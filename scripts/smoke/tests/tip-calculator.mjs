// Two controls over one field as successive stages: the dual-bound editor.
...
export const run = async ({ ev, assertEq, sleep }) => {
  assertEq(await ev(tipLine), 'Tip: 15%', 'the seeded tip renders through both stages')
  ...
  await sleep(50)
  assertEq(await ev(tipLine), 'Tip: 25%', 'the range write reached the model')
