...
export const run = async ({ ev, assertEq }) => {
  ...
  assertEq(text.includes('Your current bid: $0'), true, 'mvu seed renders the bid readout at t=0')
  assertEq(text.includes('Highest bid so far: $0'), true, 'feedback seed primes the loop: the high-water readout renders at t=0')
