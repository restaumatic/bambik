// focusVariant end-to-end: the money sub-family (refund, payout) detours
...
// while the background audit case passes straight to the fold, untouched.
...
export const run = async ({ ev, assertEq, sleep }) => {
  ...
  assertEq(await ev(`document.body.textContent.includes('0 register counts today')`), true, 'seeded audit count renders')

  await ev(click('Count the register'))
  await sleep(400)
  ...
  assertEq(await ev(`document.body.textContent.includes('1 register counts today')`), true,
    'the audit folded straight into the model')
  ...
  assertEq(await ev(`document.body.textContent.includes('Till balance: €200')`), true,
  ...
  assertEq(await ev(`document.body.textContent.includes('Till balance: €175')`), true,
  ...
  await ev(`document.querySelector('.mdc-dialog--open .mdc-dialog__button').click()`)
  await until(`document.body.textContent.includes('Till balance: €165')`,
    'the second wrapped case confirms through its own dialog')
  assertEq(await ev(`document.body.textContent.includes('1 register counts today')`), true,
    'money movements never touched the audit count')
}
