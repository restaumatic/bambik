...
dependency (imported across `PUI.Data.Profunctor.Row` and the
`PUI.Data.Profunctor.Row.*` direction modules).
...
- **Total elimination is *not* worth swapping:** `case_ # on … # on …` over a
  ...
  case. So the remaining `case_`/`on` eliminators in `demo/1/Main.purs`
