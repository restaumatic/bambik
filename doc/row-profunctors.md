...
- Row strengths and their combinators (each beside its merge):
  - `RecordToRecord.purs` — `focusRecord` (on `Strong`); `property` (on `Strong`) / `field` (on bare `Profunctor`); `tapped` (on `Strong` — the display tap)
