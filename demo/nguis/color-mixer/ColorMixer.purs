...
import PUI (asField, completed, displayed, forValue, mvu, projection, tapped, toCase, updates)
import PUI.HTML (body, clicked, div, dynamic, each, text, (:=))
          ...
          ( div >>> "style" := "margin: 10px 0;" $
              ( dynamic \m -> Semigroupoid.do
                  ...
                  div >>> "style" := "display: flex; gap: 8px; margin-top: 10px;" $
                    each palette \p ->
                      lcmap (const p.name) $ clicked $ lcmap (const {})
                        ( div >>> "title" := p.name
                            >>> "style"
