...
circleDrawerFluent =
  body $
    card { caption: "Circle Drawer" } $ ( Semigroupoid.do
        slider { label: "Diameter" } # asField @"diameter" # provided selectedDiameter # updated (informed adjustDiameter)
        ...
        ( div $ RecordToVariant.do
            ...
            button { label: "Redo" } # asCase @"redo") # updated (match { undo: const <<< undo, redo: const <<< redo })
    ) # mvu emptyCanvas
