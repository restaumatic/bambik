    ...
    ( svg >>> "viewBox" := "0 0 500 300" >>> "style" := "border: 1px solid #ccc; display: block; margin: 10px 0; background: white; width: 100%; max-width: 500px; height: auto; touch-action: none;" $
      ( onClickedXY @"picked"
        ( ( circle >>> "stroke" := "#333" >>> attrWith "cx" _.x >>> attrWith "cy" _.y >>> attrWith "r" _.r
          >>> attrWith "fill" circleFill $ blank ) # foreach @"key" canvasCircles ) )) # updated (match { picked: selectOrAddCircle })
    ( div $ RecordToVariant.do
      button @"Undo" (staticText "Undo")
      button @"Redo" (staticText "Redo") ) # updated (match { "Undo": const <<< undo, "Redo": const <<< redo })
  ) # mvu emptyCanvas
