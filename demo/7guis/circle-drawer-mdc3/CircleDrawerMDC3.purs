  ...
  body $
    elevation5 $
      card { caption: "Circle Drawer" } $ ( Semigroupoid.do
          sliderLive { label: "" } # asField @"value" @"diameter" # provided selectedDiameter # updated (informed adjustDiameter)
          ...
          ( cardActions $ RecordToVariant.do
              button { label: "Undo", icon: "undo" } # asCase @"clicked" @"undo"
              button { label: "Redo", icon: "redo" } # asCase @"clicked" @"redo") # updated (match { undo: const <<< undo, redo: const <<< redo })
      ) # mvu emptyCanvas
