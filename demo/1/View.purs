...
order = T.do
  ...
  elevation20 $ S.do
    caption $ text # constant "Order " <> debouncer' >>> value >>> shortId
    card S.do
  ...
  arr (\submittedOrder -> { authToken: "", submittedOrder })
  confirmationDialog { title: text # constant "Submit order " <> value >>> shortId >>> submittedOrder <> constant "?", dismiss: text # constant "No", confirm: text # constant "Yes" } >>> spy "dialog" $ S.do
