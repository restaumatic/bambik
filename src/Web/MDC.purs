...
containedButton { label } =
  Web.button (classes "mdc-button mdc-button--raised initAside-button") mempty $ bracket (getCurrentNode >>= newComponent material.ripple."MDCRipple") (const $ pure) (const $ pure) $ clickable S.do
...
filledTextField { floatingLabel } value =
  label (classes "mdc-text-field mdc-text-field--filled mdc-text-field--label-floating") mempty $ bracket (getCurrentNode >>= newComponent material.textField."MDCTextField") (const $ pure) (const $ pure) S.do
    span (classes "mdc-text-field__ripple") mempty mempty
    span (classes "mdc-floating-label" <> attr "id" "my-label-id") (\currentInput -> if not (null currentInput) then classes "mdc-floating-label--float-above" else mempty) (floatingLabel >>> hush)# value
    input (classes "mdc-text-field__input" <> attr "type" "text" <> attr "aria-labelledby" "my-label-id") # value
    span (classes "mdc-line-ripple") mempty mempty

checkbox :: forall a b. { labelContent :: Widget a b } -> (Widget Boolean Boolean -> Widget a a) -> Widget a a
checkbox { labelContent } checked =
  div (classes "mdc-form-field") mempty $ bracket (getCurrentNode >>= newComponent material.formField."MDCFormField") (const $ pure) (const $ pure) S.do
    div (classes "mdc-checkbox") mempty $ bracket (getCurrentNode >>= newComponent material.checkbox."MDCCheckbox") (const $ pure) (const $ pure) S.do
      ...
      div (classes "mdc-checkbox__ripple") mempty mempty
...
radioButton { labelContent } value =
  div (classes "mdc-form-field") mempty $ bracket (getCurrentNode >>= newComponent material.formField."MDCFormField") (const $ pure) (const $ pure) S.do
  div (classes "mdc-radio") mempty $ bracket (getCurrentNode >>= newComponent material.radio."MDCRadio") (const $ pure) (const $ pure) S.do
      ...
      div (classes "mdc-radio__ripple") mempty mempty
  label (attr "for" id) mempty (labelContent >>> hush)
    where
      id = unsafePerformEffect randomElementId
...
dialog { title } content =
  aside (classes "mdc-dialog") mempty $ bracket initializeMdcDialog openMdcComponent closeMdcComponent S.do
    ...
    div (classes "mdc-dialog__scrim") mempty mempty
    where
