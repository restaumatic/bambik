...
withRecordDefault :: forall l p a r o.
  ...
  Cons l a () r =>
  Profunctor p =>
  p { | r } o -> a -> p {} o -- can be placed in any record-to-*
...
withRecordOutputDefault :: forall l p a r i.
  ...
  Cons l a () r =>
  Profunctor p =>
  p i {} -> a -> p i { | r } -- can be placed in any *-to-record
...
-- variant `[ l :: a ]` already carries an `a`, so no value is needed.
-- Lifts `p a o` into `p [ l :: a ] o`.
tagVariantInput :: forall l p a r o.
  RL.RowToList r (RL.Cons l a RL.Nil) =>
  IsSymbol l =>
  Cons l a () r =>
  Profunctor p =>
  p a o -> p [ | r ] o -- can be placed in any variant-to-*
...
-- Lifts `p i a` into `p i [ l :: a ]`.
tagVariantOutput :: forall l p a r i.
  RL.RowToList r (RL.Cons l a RL.Nil) =>
  IsSymbol l =>
  Cons l a () r =>
  Profunctor p =>
  p i a -> p i [ | r ] -- can be placed in any *-to-variant
