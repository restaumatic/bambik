...
-- | coroutine step**, the dual of `RecordToVariant`'s `IteratingRecordToVariant`. `resuming`
...
-- |
-- | ```
-- | resuming :: p a b -> p (Either a c) (Tuple b c)
-- |                        -- Left  a = fresh input
...
class Profunctor p <= ResumingVariantToRecord p where
  resuming :: forall a b c. p a b -> p (Either a c) (Tuple b c)

-- | Single-case specialization of `resuming` — the `edit`-position combinator
-- | for this direction (the dual of `iterateProperty`). It threads one label
