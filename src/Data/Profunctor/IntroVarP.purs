...
class Profunctor p <= IntroVarP p where
  liftIntroVar :: forall s a. p s a -> p s (Either a s) -- s must not be Void

-- `IntroVarO s t a` encodes `Either a s -> t`
type IntroVarO s t a = forall p. IntroVarP p => Optic p s t s a

introVar :: forall s t a. (Either a s -> t) -> IntroVarO s t a
introVar introduce = liftIntroVar >>> rmap introduce

instance IntroVarP (Cont r) where
  -- :: [(b -> r) -> (Unit -> r)] -> (Either b s -> r) -> s -> r
  -- s may be Void
  liftIntroVar r = wrap \bs2r s -> bs2r (Right s)
