...
class Profunctor p <= IntroPropP p where
  liftIntroProp :: forall s a . p s a -> p s (Tuple a s) -- a introduced, s preserved
...
-- `IntroPropO s t a` encodes `Tuple a s -> t`
type IntroPropO s t a = forall p. IntroPropP p => Optic p s t s a

introProp :: forall s t a. (Tuple a s -> t) -> IntroPropO s t a
...
introProp' = introProp (\(Tuple b s) -> insert (Proxy @l) b s)

instance IntroPropP (->) where
  liftIntroProp f s = Tuple (f s) s

-- Useful instance for decoding half-lenses
instance IntroPropP (Cont r) where
  liftIntroProp callbacksb = wrap $ \bs2r s -> unwrap callbacksb (\b -> bs2r (Tuple b s)) s
