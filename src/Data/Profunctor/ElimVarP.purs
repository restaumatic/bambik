...
class Profunctor p <= ElimVarP p where
  liftElimVar :: forall s a. p a s -> p (Either a s) s -- a eliminated, s preserved
...
-- `ElimVarO s t a` encodes `s -> Either a t`
type ElimVarO s t a = forall p. ElimVarP p => Optic p s t a t

elimVar :: forall s t a. (s -> Either a t) -> ElimVarO s t a
elimVar eliminate = liftElimVar >>> lcmap eliminate

elimVarInverse :: forall s t a. ElimVarO s t a -> s -> Either a t
elimVarInverse f = unwrap (f (Cont (const Left))) Right
