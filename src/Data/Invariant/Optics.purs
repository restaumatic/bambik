...
invLens :: forall i a s. Invariant i => InvCartesian i => (s -> a) -> (s -> a -> s) -> i a -> i s
invLens get set ia = invmap (\(Tuple a s) -> set s a) (\s -> Tuple (get s) s) (invfirst ia)

invPrism :: forall i a s. Invariant i => InvCocartesian i => (a -> s) -> (s -> Either a s) -> i a -> i s
invPrism review preview ia = invmap (\aors -> either review identity aors) preview (invleft ia)

invProjection :: forall i a s . Invariant i => InvCartesian i => (s -> a) -> i a -> i s
...
invZero :: forall i a s . Invariant i => InvPlus i => i a -> i s
invZero = const invzero
...
invConst :: forall i a s. Invariant i => InvCartesian i => a -> i a -> i s
