...
- **src/PUI.purs** — the core profunctor and the carrier-independent algebra's
  ...
  **development diagnostics** — the private `tr` (emission trace) and
  ...
  | `tapped` | the display tap: show the value flowing through a stage and pass it on — forwarded unconditionally at feed time, so a detached pane, an empty collection or a pipeline tail never withholds; record-typed and subsuming. Accepts only a component whose output is `{}` — a display's output — so an editor inside a tap fails to unify, never silently swallows an edit; `observed` accepts only a `{}`-output status the same way. **Derived, not primitive**: `tapped w = recordToRecord w identity` — the merge with the echo wire (the duoidal comultiplication), lawful because the gates pre-satisfy zero-field sides; lives in `Data.Profunctor.Row.RecordToRecord`, carrier-agnostic |
  | `muted` | the counit: render, and **deliberately discard** the component's output (`rmap (const {})`) — the visible form of what no stage may do silently; `# muted # tapped` makes a genuinely emitting assembly (a `foreach` forwarding its elements, a packaged collection display echoing its array) usable as a pure display |
  | `observed` | `tapped`'s `+`-diagonal sibling: every event forwards once at feed time; the status's own emissions are dropped (events are one-shot) |
...
- `tapped` - live views as pipeline stages (slider readouts, summary lines, data tables): display every value fed, pass it on unconditionally; the read function is closed at its own footprint (`tapped` subsumes)
...
`PUI Web`: `tapped (dynamic …)` over element oculars, since structure
...
`# providedCase` button panes and a keyed-`foreach` lap list `# tapped`
— where `tapped`'s unconditional forwarding *is* the sequence merge's announcing
