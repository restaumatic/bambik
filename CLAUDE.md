...
- **src/PUI.purs** — the core profunctor and the carrier-independent algebra's
  ...
  equivalence; interchange fails on the nose and holds as `⊑`, so a gated
  merge is premonoidal, not monoidal). Also `Hosting` and
