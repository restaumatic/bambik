...
- **src/PUI/Web/HTML.purs** — the 1-1 HTML vocabulary over the carrier.
  ...
  **Interaction vocabulary** (the collection combinators `foreach`/`edited`/
  ...
  | gated display rungs (RESEARCH) | `shown @l f` / `told line` (instant), `shownAs proj content` (ambient structured content, registered at build), `shownWhen proj content` / `shownCase @l f content` (panes), `shownEach @l proj item` (keyed collection) — each `p { o \| rest } { o \| rest }`, releasing the fed row per its policy; `confirmed` (the witness rung) lives in the design systems |
