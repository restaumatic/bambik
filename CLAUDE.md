...
- **src/PUI.purs** — the core profunctor and the carrier-independent algebra's
  ...
  **development diagnostics** — the private `tr` (emission trace) and
  ...
  | adopter family | `asField`/`forProperty`/`atField`/`field`/`subStrong`/`subChoice`/`toCase`/`toCases`/`atCase`/`forCase`/`forCases`, plus `acted`/`optioned` — the view-side read adopters `projection`/`projected` are DELETED (2026-08-31, the presentation-model rule: displays are verbatim, formatting is a `settled` invariant in logic — doc/research-presentation-model.md; `projected` survives only as vocabulary-internal plumbing in the statuses). `forCase @l f` is the derived single-case status convenience, `forCases { l: f }` by law — kept for the `@l` grammar (restored same day after a brief dissolution); `forCases` takes the record of per-case copy functions directly, no `match` at the call site |
...
- **extras/row-profunctor/Data/Profunctor/Row/** - Row profunctors over `Record`/`Variant`: four direction modules, each carrying its **direction class** — the binary merge, the one genuine per-carrier primitive; no class carries a unit of its own: the unit laws are conditional on the carrier (*if* `p` is a `Category`, `identity` at the unit object must play well with the merge — `×→×`, `+→+` — and so must that wire entered from the empty variant for `+→×`, `lcmap case_ identity`), and only `×→+` keeps a class-member unit, `silence`, the one no wire reaches — with qualified-do sugar (`bind`/`discard`). Everything kept is reached by a demo or a law test (L14); laws are stated in the module headers. Type variables follow the photographic schema: focus `f`, background `b`, shot `s` (`Cons l f b s`), reality `r`.
  - **`RecordToRecord.purs`** (×→×) — merge `recordToRecord` (`SharedRecordInputs` + `OwnedRecordOutputs`; gated on `PUI`, zero-field sides pre-satisfied and inert — `{}` is always known and a contribution of zero fields is no contribution);
    ...
    over bare `Profunctor`: `muted` (the counit — render and deliberately discard, `rmap (const {})`; the explicit word the gated content slots' `{}` demand points to); `forProperty` (read a field of a **wider** row into the display, the label derived from the leaf's row — the display-side `field @l` for context-pinned rows (collection items, pane payloads): `text @"label" # forProperty` — the one surviving read adopter, selection not formatting; `projected`, the whole-value read, is library plumbing for the statuses and not re-exported from `PUI`);
