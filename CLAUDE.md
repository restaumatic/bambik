...
- **extras/row-profunctor/Data/Profunctor/Row/** - Row profunctors over `Record`/`Variant`: four direction modules, each carrying its **direction class** — the binary merge plus its nullary unit `pempty`, the genuine per-carrier primitives — with qualified-do sugar (`bind`/`discard`). Everything kept is demo-reachable; laws are stated in the module headers. Type variables follow the photographic schema: focus `f`, background `b`, shot `s` (`Cons l f b s`), reality `r`.
  - **`RecordToRecord.purs`** (×→×) — merge `recordToRecord` (`SharedRecordInputs` + `OwnedRecordOutputs`; gated on `PUI`, zero-field sides pre-satisfied — `{}` is always known);
    ...
    over bare `Profunctor`: `atProperty` (`atField`'s open-row sibling — feed a structural UI component the bare field `l` of a wider row, exactly as `forProperty` reads a wider row's field into the display), `muted` (the counit — render and deliberately discard, `rmap (const {})`; the explicit word the gated content slots' `{}` demand points to); `toField` (introduce a **bare** output as field `l` — `rmap`-only, the transpose of `toCase`; the output-side canonical rename is `toField @l _.value`, as label-indexed emitters are `recordToCase @l` at the leaf); `forProperty` (read a field of a **wider** row into the display, the label derived from the leaf's row — the display-side `field @l` for context-pinned rows (collection items, pane payloads): `text @"label" # forProperty identity`; `projection`'s label-preserving retype is the narrow-row form);
...
- variant editing - **record-shaped editor state**: the model keeps the variant, the editor keeps every payload; `bracketed <stateOf> <caseOf>` wraps `Semigroupoid.do { selection component; payload panes }` (the variant in via a state function seeding absent payloads, out via a projection on the selection, self-traced in between) — each pane a payload-typed editor stage `# provided <paneOf> # updated <setPane>`, shown by the presence of its `Maybe` payload; consistency via the self-trace re-broadcast; unit-payload variants need only the bracket around one selection component
