...
- **src/MDC.purs** - Material Design Components as oculars and as **row-profunctor merges**: every compound is a `RecordToVariant.do` (`×→+`, the ungated direction — silent chrome needs no announcement) over the `Data.Profunctor.Row` bridges `live`/`watching` with boundary `scalar`/`backdrop`; chrome is self-pinning (`staticText`/`staticHTML`/`pempty` are typed `{} → []`); code order = DOM order
...
- **src/Data/Profunctor/Row/** - Row profunctors over `Record`/`Variant`. Each of the four direction modules is organized in three layers: **strength** (the unary power — ecosystem `Strong`/`Choice` on the diagonals, module-defined `Resolving`/`Retaining` on the mixed directions) → **direction class** (the binary merge plus its nullary unit `pempty`, the genuine per-carrier primitives) → **free functions over the strength** (everything else — no row-focus classes; laws pinning the unary to the merge are stated in the module headers: identity-pinned on the diagonals, silence-pinned on the mixed directions). Type variables follow the photographic schema: focus `f`, background `b`, shot `s` (`Cons l f b s`), reality `r`.
  ...
  - **`Row.purs`** (module `Data.Profunctor.Row`) — the shared floor: the row-constraint vocabulary (`InclusiveRows`/`ExclusiveRows`/`DispatchableVariants`) + the `dimap`-only unary **reshapings** (`widenRecordInput`/`narrowVariantInput`/`narrowRecordOutput`/`widenVariantOutput` and single-label forms `widenInputProperty`/`widenOutputCase`/`narrowInputCase`/`narrowOutputProperty`) + the scalar↔singleton-row **bridges** (`live`/`watching`/`scalar`/`backdrop` — compound widgets as `×→+` merges; chrome needs no bridge, the static leaves are typed `{} → []`); the merge instances in `UI.purs` build on the two widening ones.
...
- `live` / `watching` / `scalar` / `backdrop` (`Data.Profunctor.Row`) - the scalar↔singleton-row bridges: compound widgets as `×→+` merges (self-pinning chrome + one live piece), code order = DOM order
...
- **demo/2/** - Plain HTML demo: the whole page as one `×→+` merge — self-pinning statics around a minimal `×→×` record merge over plain `input`s
