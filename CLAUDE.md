...
- **src/PUI.purs** — the core profunctor and the carrier-independent algebra's
  ...
  laws are about **kind, not shape** — `bracketed`'s variant editor echoes
  lawfully (a variant-shaped behavior), `clicked` never does (a
  record-shaped event); shape and kind coincide for every published leaf,
  and the law is a protocol obligation the present type cannot enforce.
  **Stateful
...
- **extras/row-profunctor/Data/Profunctor/Row/** - Row profunctors over `Record`/`Variant`: four direction modules, each carrying its **direction class** — the binary merge, the one genuine per-carrier primitive; no class carries a unit of its own: the unit laws are conditional on the carrier (*if* `p` is a `Category`, `identity` at the unit object must play well with the merge — `×→×`, `+→+` — and so must that wire entered from the empty variant for `+→×`, `lcmap case_ identity`), and `×→+`, the one no wire reaches, has no unit at all — only the empty-merge law for any silent element, tested on an inline probe (`silence`, its former class-member unit, deleted 2026-09-11) — with qualified-do sugar (`bind`/`discard`). Everything kept is reached by a demo or a law test (L14); laws are stated in the module headers. Type variables follow the photographic schema: focus `f`, background `b`, shot `s` (`Cons l f b s`), reality `r`.
