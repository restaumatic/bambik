...
- **src/PUI.purs** — the core profunctor and the carrier-independent algebra's
  ...
  **development diagnostics** — the private `tr` (emission trace) and
  ...
  | `tapped` | the display tap: show the value flowing through a stage and pass it on — forwarded unconditionally at feed time, so a detached pane, an empty collection or a pipeline tail never withholds; record-typed and subsuming |
