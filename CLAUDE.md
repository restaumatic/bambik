...
- **src/PUI.purs** — the core profunctor and the carrier-independent algebra's
  ...
  is why `Looping` is its own class and the row trace forms take seeds.
...
- **src/PUI/Gate.purs** — the output gate of the two record-output merges as
  a **pure Mealy machine**: `gateStep :: GateConfig -> GateState -> GateInput
  -> Tuple GateState GateOutput`, in a module with no `Effect` in it. The
  effectful part is one function in `PUI`, `driveGate` (read, step, write,
  act); `steppedFeed` brackets a feed's broadcast with `StepBegun`/`StepEnded`
  and `gatedRecordOutputs` turns each operand emission into a `Contributed`
  input. The machine is data-independent with finite control, which is what
  makes the bounded exhaustive check in test/Exhaustive.purs complete rather
  than a sample (doc/observational-semantics.md §9), and it is the port
  target for a mechanised proof.
...
- **src/PUI/Web/HTML.purs** — the 1-1 HTML vocabulary over the carrier.
  ...
  **Interaction vocabulary** (the collection combinators `foreach`/`edited`/
  ...
  | `clicked @l f` | click emitter for any element, emitting `f` of the row last fed as case `l` (`button`'s replay-last-value protocol — replay is lawful over records only, so the payload is a row, and the channel is a variant, so the source is `×→+` by shape); its **content subsumes** — a multi-reader content states its row once in a named closed *face* function |
...
- **No canonical labels: leaves state business labels, adopters derive them** (L3).
  ...
  The closure of the label discipline is the **anchor invariant** (writing.md *The anchor invariant*; library-side face and rejected-design record in guardrails L3): every view line names exactly one semantic anchor — a model **field** (`@l` on an editor/selector), a business **case** (`@l` on an emitter, pane or status), a **named read function** (a display's content), or **nothing** (chrome) — line ↔ named symbol in the model or logic module, deliberately never line ↔ field (a symbol on an ocular was rejected 2026-09-03: no model, nothing to anchor)
...
- **extras/row-profunctor/Data/Profunctor/Row/** - Row profunctors over `Record`/`Variant`: four direction modules, each carrying its **direction class** — the binary merge, the one genuine per-carrier primitive; no class carries a unit of its own: the unit laws are conditional on the carrier (*if* `p` is a `Category`, `identity` at the unit object must play well with the merge — `×→×`, `+→+` — and so must that wire entered from the empty variant for `+→×`, `lcmap case_ identity`), and only `×→+` keeps a class-member unit, `silence`, the one no wire reaches — with qualified-do sugar (`bind`/`discard`). Everything kept is reached by a demo or a law test (L14); laws are stated in the module headers. Type variables follow the photographic schema: focus `f`, background `b`, shot `s` (`Cons l f b s`), reality `r`.
  - **`RecordToRecord.purs`** (×→×) — merge `recordToRecord` (`SharedRecordInputs` + `OwnedRecordOutputs`; gated on `PUI`, zero-field sides pre-satisfied and inert — `{}` is always known and a contribution of zero fields is no contribution — and **released once per feed**: the broadcast is one step, so a feed changing several fields emits one fresh row, never a torn one);
    ...
    over bare `Profunctor`: `muted` (the counit — render and deliberately discard, `rmap (const {})`; the explicit word the gated content slots' `{}` demand points to); `forProperty` (read one field of a **wider** row into a *labelled* leaf, the label derived from the leaf's row — the display-side `field @l` for context-pinned rows: selection, never formatting, since copy is a read function at the leaf; the whole-value `projected` is DELETED, the statuses reading their payload through the vocabulary-internal `textOf`);
...
`snackbar`), movie-browser (`tabBar` + `filterChip` filters over one
