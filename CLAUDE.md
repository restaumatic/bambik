...
- **src/PUI.purs** — the core profunctor and the carrier-independent algebra's
  ...
  (feed-idempotence, record-echo totality, no synchronous variant echo), and
  ...
  up to stutter for protocol-respecting operands — so a gated merge is
  premonoidal counting renderings and monoidal-up-to-stutter counting
  channels; `>>>` and the merges are monotone in `⊑`). **Stateful
