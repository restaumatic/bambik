...
- **src/PUI/Gate.purs** — the knowledge gate as **one pure Mealy machine**
  ...
  retain-last thereafter. The two record-output merges enrol their **owned
  field labels** (2026-09-15: participants are fields, not sides, so the
  ...
  the row (`Record.Unsafe`, `Union o1 o2 o` the witness), `actedWith`
...
- **src/PUI/Web/HTML.purs** — the 1-1 HTML vocabulary over the carrier.
  ...
  **Entry**: `body :: PUI Web {} o -> Effect Unit` registers the wiring and
  ...
  initial`/`mvu seed` down to `{}`, the one self-pointed record. A forgotten seed is therefore a
