...
- **No canonical labels: leaves state business labels, adopters derive them** (L3).
  ...
  **Editors are only partly converted**: ~42 editor sites still pass explicit copy over a camelCase label (`slider @"tipPercent" { label: "Tip percentage" }`, `textField @"celsiusText" { label: "Celsius" }`) — legitimate wherever the caption genuinely differs from the field (units, the two-label temperature case, the `formula` hint), but elsewhere just unconverted; the label-is-the-copy rule is fully enforced on emitters and on the editors the field rename reached.
