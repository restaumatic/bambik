...
- **No canonical labels: leaves state business labels, adopters derive them** (L3).
  ...
  For **editors** the same rule holds, and the caption config (`label:`/`floatingLabel:`) now survives only where the caption genuinely cannot be the field name — 38 sites over 12 fields, each of a principled kind: a **format hint** (`@"start"`/`@"return"` → "Start/Return date (DD.MM.YYYY)", `@"formula"`, `@"prefix"`), a **unit** (`@"amount"` → "Amount (€)", `@"years"` → "Term (years)", `@"bid"` → "Your bid ($)"), a **disambiguating rename** (order-form's `@"shortId"`/`@"orderId"` → "Short ID"/"Unique ID", order-dashboard's `@"window"` → "Showing"), or the **two-label case** where the read field differs from the typed-text payload (`@"celsiusText"`/`@"fahrenheitText"` → "Celsius"/"Fahrenheit"). Copy that is merely a plain phrase belongs on the label instead (`toggleSwitch @"Takeaway cup" {}`, `slider @"Tip percentage" {}`, `filledTextField @"What needs to be done?" {}`).

  The one two-label case — an editor whose read field differs from its typed-text payload (temperature-converter) — reads through a named face function via `# projected` with a distinct payload label (`@"celsiusText"`)
