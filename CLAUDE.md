...
- **No canonical labels: leaves state business labels, adopters derive them** (L3).
  ...
  For **editors** the same rule holds, and the config now survives only where the caption genuinely cannot be the field name — 22 sites, every one of a principled kind: a **format hint** (`textField @"start" { label: "Start date (DD.MM.YYYY)" }`, `@"formula"`, `@"prefix"`), a **unit** (`sliderLive @"amount" { label: "Amount (€)" }`, `@"years"`, `@"bid"`), or the **two-label case** where the read field differs from the typed-text payload (`@"celsiusText"`). Copy that is merely a plain phrase belongs on the label instead (`toggleSwitch @"Takeaway cup" {}`, `slider @"Tip percentage" {}`).
