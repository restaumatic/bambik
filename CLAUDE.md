...
- **src/PUI.purs** — the core profunctor and the carrier-independent algebra's
  ...
  **development diagnostics** — the private `tr` (emission trace) and
  ...
  | `displayed` | make any display an unconditional pass-through stage; for a terminal collection its echo *is* the sequence merge's announcing unit |
  | `observed` | `displayed`'s `+`-diagonal sibling: every event forwards once at feed time; the status's own emissions are dropped (events are one-shot) |
  | `tapped` | a display tap — show the value flowing through a stage and pass it on; record-typed and subsuming |
  | `completed` | complete a component's output to its full input row, unproduced fields carried from the retained input |
  | `required` / `optional` | mark a type-changing selector always-selected / possibly-unselected (label derived from the leaf's row); `optional` keeps the `Maybe`, so an unmade choice is honest knowledge rather than a starved gate |
  | `every` | the heartbeat wire: pass-through plus a periodic step over a sub-row, merged back over the last value |
  | adopter family | `asField`/`projection`/`projected`/`forProperty`/`atField`/`atProperty`/`field`/`toField`/`subStrong`/`subChoice`/`toCase`/`toCases`/`atCase`/`forCase`/`forCases`, plus `acted`/`optioned` |

  Durations are structural `{ ms :: Number }` throughout (`every`, `debounced`,
  `resolveFor`, `inputDebounced`, `debouncedTextField`) — never `Milliseconds`.
...
- **src/PUI/Web/HTML.purs** — the 1-1 HTML vocabulary over the carrier.
  ...
  **SVG** oculars (`svg`/`circle`/`path`/`text`) live in **`PUI.Web.SVG`**,
  ...
  self-dismissing). `radioButton`/`checkboxInput` stay scalar optic-position
  ...
  **Structure computed from data is `PUI Web` all the way down** — no markup
  ...
  2. **Structure that genuinely varies with the data** (markdown blocks): the
     closure builders `foreachWith` (a whole component per element,
     `el ("h" <> show level)`), `dynamic` (its single-value case) and
     `each xs build` (a closure-known list pinned to `{}`) rebuild per feed.
...
- **Two sorts.** *Components* carry a model interface and are citizens of
  ...
  *Oculars* are shape-preserving decorators with no model of their own
  (`card`/`cardActions`, dialogs, lists, typography, elevations).
...
- **The `dimap` round-trip contract for editors** (stated in each module
  ...
  failing conversions belong in the model (`settled` after `completed`), never
  in a leaf bracket.
...
Internals (MDC2/MDC3): the live leaf is `field @l`-lifted (preferred over raw
`focusProperty` for annotation-free inference; runtime-exactness is enforced by
the merge gates' `MergeableRecords` trim) with hand-fused chrome where abstract
...
- **No canonical labels: leaves state business labels, adopters derive them** (L3).
  ...
  For **editors** the same rule holds, and **no demo passes a caption config at all**: the label carries the full copy, punctuation and units included — `filledTextField @"Start date (DD.MM.YYYY)" {}`, `sliderLive @"Amount (€)" {}`, `filledTextField @"Formula (e.g. =SUM(A0:A5)*2)" {}`, `filledTextField @"What needs to be done?" {}`. The type argument *is* the caption, so a leaf never states its copy twice. Selector **options** follow the same rule via `choices` (`class Choices` in `PUI.Web`): a choice states its copy once, at its case, and the `{ value, label }` echo disappears — `dropdown @"Room" {} (choices (Proxy @"Focus pod (4 seats)" /\ Proxy @"Boardroom (12 seats)"))`. The list is a **nested tuple of proxies rather than the variant row itself**, deliberately: the compiler sorts a row alphabetically, while option order is a design decision (rooms by size, durations by length), so the tuple is what preserves declaration order. The vocabulary still keeps `floatingLabel:`/`label:` in its signatures for real copy a label genuinely cannot be — localized wording above all.

  The one two-label case — an editor whose read field differs from its typed-text payload (temperature-converter) — reads through a named face function via `# projected`, and **all three concepts take distinct names**: the payload label is the caption (`@"°C"` — a label is any string, so where a symbol *is* the conventional caption it is written as one), the model quantity is `celsiusReading`, and the reader formatting one into the other is `celsiusText`. Sharing a word across two of them is what made an earlier mechanical rename corrupt this demo, so the names are deliberately unique
...
- **extras/row-profunctor/Data/Profunctor/Row/** - Row profunctors over `Record`/`Variant`: four direction modules, each carrying its **direction class** — the binary merge plus its nullary unit `pempty`, the genuine per-carrier primitives — with qualified-do sugar (`bind`/`discard`). Everything kept is demo-reachable; laws are stated in the module headers. Type variables follow the photographic schema: focus `f`, background `b`, shot `s` (`Cons l f b s`), reality `r`.
  - **`RecordToRecord.purs`** (×→×) — merge `recordToRecord` (`SharedRecordInputs` + `OwnedRecordOutputs`; gated on `PUI`);

    over ecosystem `Strong`: `subStrong` (sub-record focus, background carried), `focusProperty` (the type-changing field lens);

    over the **unit**: `announce` (its `rmap`-closure — the announcing constant) and `with` (`announce a >>> w` over `Semigroupoid` — discharge the initial-state obligation) with `mvu` (`with seed (looped w)` over `Looping` — the app shape, closed to `{}`), plus the subsuming `settled` (`rmap`-only normalization over a stated sub-row footprint) and `informed` (the dispatch adapter — pure record algebra, payload laid over model row, left-biased);

    over bare `Profunctor`: `atProperty` (`atField`'s open-row sibling — feed a structural UI component the bare field `l` of a wider row, exactly as `forProperty` reads a wider row's field into the display), `toField` (introduce a **bare** output as field `l` — `rmap`-only, the transpose of `toCase`, and the output half `field` fuses: `field @l = atField @l <<< toField @l identity`; the output-side canonical rename is `toField @l _.value`, as label-indexed emitters are `recordToCase @l` at the leaf), `field` (`focusProperty`'s closed-singleton form — the merge-operand/nesting shape, `dimap`-only, annotation-free under the merges; runtime-exactness is enforced by the merges themselves — their `MergeableRecords` evidence trims every operand emission to its declared output row before the gates' left-biased `Record.union`, so lens-rebuilt emissions carrying stale sibling fields can't shadow fresh values); `required` (marks a type-changing selector as always-selected, its label derived from the leaf's row: `select @l config options # required`); `forProperty` (read a field of a **wider** row into the display, the label derived from the leaf's row — the display-side `focusProperty` for context-pinned rows (collection items, pane payloads): `text @"label" # forProperty identity`; `projection`'s label-preserving retype is the merge-operand form); `tapped` (a `Strong`-derived display tap: shows the value flowing through a pipeline stage and passes it on — the display's echo triggers the forwarding; honest over displays only; **record-typed and subsuming**, so `text @"summary" # projected readout # tapped` works with `readout` closed at just the fields it formats); `completed` (complete a UI component's output to its full input row over `Strong` — unproduced fields carried from the retained input, `FieldNames`-trimmed union, absorbs the input widening; kills `field @l identity` echo wires);
...
| circle-drawer | **channel-fed SVG canvas** — built once, updated via `attrWith`; container-level `onClickedXY`; live-preview resize via `sliderLive` with an `adjusting` flag coalescing a drag into one undo transaction |
...
`×→×` form (nested merges; variant editors as `bracketed` pipelines of
`tabBar`/`segmentedButton` + `provided` payload panes) → `tapped` live summary
...
| cashbox | `subChoice` — selective interception as UX: outgoing money detours through confirmation dialogs, incoming posts straight to the fold; every branch exactly `informed`-dispatched; payloads via `button @l {…} # with patch` |
...
`segmentedButton` filter), tip-calculator (all-`×→×`, sliders, `tapped` money
...
`PUI Web`: `displayed (dynamic …)` over element oculars, since structure
...
`# providedCase` button panes and a keyed-`foreach` lap list `# displayed`
— where `displayed`'s unconditional echo *is* the sequence merge's announcing
unit, so an empty lap list never starves the gate),
...
attendees slider *born* with the chosen room's capacity, so an incomplete
meeting is unbookable by construction), loan-calculator (Bootstrap, all
`sliderLive`).
